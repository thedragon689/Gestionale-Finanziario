...
router.post('/start', async (req, res) => {
  try {
    ...
    const result = await scheduler.runManualSimulation(simulationDate);
...
router.get('/stats', async (req, res) => {
  try {
    ...
    const stats = await simulationService.getSimulationStats(start, end);
...
router.get('/assets', async (req, res) => {
  try {
    ...
    const performance = await simulationService.getAssetPerformance(symbol, parseInt(days));
...
router.get('/transactions', async (req, res) => {
  try {
    ...
    const transactions = await simulationService.getUserTransactions(userId, start, end);
...
router.get('/events', async (req, res) => {
  try {
    ...
    let events;
    if (active === 'true') {
      events = await simulationService.getActiveEvents();
    } else {
...
router.post('/historical', async (req, res) => {
  try {
    ...
    const results = await scheduler.runHistoricalSimulations(start, end);
...
router.get('/report/monthly', async (req, res) => {
  try {
    ...
    const monthlyStats = await simulationService.getSimulationStats(startOfMonth, endOfMonth);
...
router.get('/dashboard', async (req, res) => {
  try {
    ...
    const recentStats = await simulationService.getSimulationStats(startDate, endDate);
    
    // Ottieni eventi attivi
    const activeEvents = await simulationService.getActiveEvents();
...
async function getSystemHealth() {
  try {
    ...
    return {
      database: 'HEALTHY',
      scheduler: scheduler.isInitialized ? 'RUNNING' : 'STOPPED',
